...
static void as7343_read_all_channels(sensor_data_t *data)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(150));  // 等待测量完成
...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    ...
    uint32_t samples = g_sensor_buffer.write_index;
    
    // 获取最新数据
    sensor_data_t latest = {0};
    if (samples > 0) {
        uint32_t idx = (samples - 1) % BUFFER_SIZE;
        if (xSemaphoreTake(g_sensor_buffer.mutex, portMAX_DELAY)) {
            latest = g_sensor_buffer.data[idx];
            xSemaphoreGive(g_sensor_buffer.mutex);
        }
    }
    ...
    len = snprintf(html, sizeof(html),
        ...
        ".data-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:20px 0;}"
        ".data-item{background:#ecf0f1;padding:15px;border-radius:5px;}"
        ".data-item h3{margin:0 0 5px 0;color:#7f8c8d;font-size:14px;}"
        ".data-item .value{font-size:18px;font-weight:bold;color:#2c3e50;}"
        ".endpoints{margin-top:30px;}"
        ".endpoints a{display:inline-block;padding:10px 20px;margin:5px;background:#3498db;color:white;"
        "text-decoration:none;border-radius:5px;}"
        "</style></head>"
        "<body>"
        "<div class='container'>"
        "<h1>🌱 AS7343 Spectrometer</h1>"
        "<p>⏱️ Samples: %" PRIu32 "</p>"
        "<div class='data-grid'>"
        "<div class='data-item'><h3>F2 (445nm)</h3><div class='value'>%u</div></div>"
        "<div class='data-item'><h3>F6 (590nm)</h3><div class='value'>%u</div></div>"
        "<div class='data-item'><h3>F7 (630nm)</h3><div class='value'>%u</div></div>"
        "<div class='data-item'><h3>FY (730nm)</h3><div class='value'>%u</div></div>"
        "<div class='data-item'><h3>CI</h3><div class='value'>%.3f</div></div>"
        "</div>"
        "<div class='endpoints'>"
        "<h2>📡 API:</h2>"
        ...
        "</body></html>",
        samples,
        latest.f2, latest.f6, latest.f7, latest.fy, latest.ci);
